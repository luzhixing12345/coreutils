...
这里的 `UBX_ARG*` 开头的是宏, 第一个位置用于参数绑定, 后面的内容会被展开为字符串用于后续的解析

argparse.h 一共提供了六种宏

- **UBX_ARG_BOOLEAN**: 适用于单参数, 例如 `-h`, 后面不需要跟其他参数信息
- **UBX_ARG_INT**: 表示参数接收的应该是一个 int 类型的数字, 例如 `-i 100`
- **UBX_ARG_STR**: 表示参数接收的应该是一个 char* 类型的字符串, 例如 `-s "hello world"` `-s nihao`
- **UBX_ARG_INT_GROUP**: 表示接收一个组, 组的数据类型是 int
- **UBX_ARG_STR_GROUP**: 表示接收一个组, 组的数据类型是 char*
- **UBX_ARG_END**: 表示结束, 添加在 options 数组的结尾
...
`UBX_argparse_describe` 的2 3 4位置的参数分别为 解析器的名字, 简要描述, 结尾描述

`UBX_argparse_init` 第三个参数为 flag 标记位, 用于控制解析时的选项, 默认传 0 即可.

- **UBX_ARGPARSE_IGNORE_WARNING**: 忽略警告
- **UBX_ARGPARSE_ENABLE_STICK**: 允许参数粘连 `-O1 -Iinclude/`
- **UBX_ARGPARSE_ENABLE_EQUAL**: 允许参数等号 `-i=123`
- **UBX_ARGPARSE_ENABLE_MULTI**: 允许多个分离参数 `-D __KERNEL__ -D __GNU__`
- **UBX_ARGPARSE_ACCEPT_MORE**: 允许参数过多, 比如group只接受 dest src, 但是传了 100 200 300
- **UBX_ARGPARSE_ENABLE_ARG_STICK**: 允许boolean类型参数粘连 `curl -Ls xxx`

您可以使用或运算将他们组合, 并传入 flag

```c
UBX_argparse_init(&parser, options, UBX_ARGPARSE_IGNORE_WARNING|UBX_ARGPARSE_ENABLE_ARG_STICK);
```

**请注意**: UBX_ARGPARSE_ENABLE_ARG_STICK 的处理和 UBX_ARGPARSE_ENABLE_STICK UBX_ARGPARSE_ENABLE_EQUAL 存在冲突, 从合理性上来说也不应该支持类似 `-Lsi=123ql=100` 的解析模式, 仅支持 UBX_ARG_BOOLEAN 类型传递, 请合理使用

---

`UBX_ismatch` 函数用于判断是否接收到了参数, 您可以采用如下写法, 应该是比较清晰直观的
...
这里提供了一个函数 `UBX_argparse_info` 用于帮助信息的输出, 建议您在 options 中添加 -h 选项并将其绑定到 UBX_argparse_info 函数, 用于辅助帮助信息的输出

最后使用 `UBX_free_argparse` 释放内存, 对于 char* 类型的参数, 即是用 `UBX_ARG_STR*` 的参数, 注意结束之后使用 free 释放内存
...
# 多个参数重复出现且未开启 UBX_ARGPARSE_ENABLE_MULTI
...
您可组合一些情况, 例如 `UBX_ARGPARSE_ENABLE_STICK | UBX_ARGPARSE_ENABLE_MULTI`
